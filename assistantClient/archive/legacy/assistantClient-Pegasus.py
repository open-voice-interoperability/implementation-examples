...
floor_manager = None
invited_agents = []  # List to keep track of all invited agents
...
def create_agent_textbox(agent_info):
    """Create a textbox for a specific agent."""
    textbox = CTkEntry(agents_frame, width=300, placeholder_text=agent_info)
    textbox.insert(0, agent_info)
    textbox.configure(state="disabled")
    textbox.grid(pady=2, sticky="ew")
    agent_textboxes.append(textbox)
    return textbox
...
def update_agent_textboxes():
    ...
    for textbox in agent_textboxes:
        textbox.destroy()
        ...
        for event in incoming_events:
            if event.get("eventType") == "publishManifests":
                ...
                if manifests:
                    ...
                    # Update invited agent status in the list
                    invited_url = manifest_service_url
                    old_status = f"Invited: {invited_url}"
                    if assistantConversationalName:
                        new_status = f"Connected: {assistantConversationalName} ({manifest_service_url})"
                    else:
                        new_status = f"Connected: {manifest_service_url}"
                    
                    update_agent_status(old_status, new_status)
