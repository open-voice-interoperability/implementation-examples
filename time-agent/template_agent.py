    ...
    def _extract_text_from_utterance_event(self, event: UtteranceEvent) -> str:
        ...
        try:
            ...
            if isinstance(features, dict):
                if 'text' in features:
                    text = _extract_from_text_feature(features.get('text'))
                    if text:
                        return text

                for key, feature in features.items():
                    if isinstance(key, str) and 'text' in key.lower():
...
def load_manifest_from_config(config_path: str = "agent_config.json") -> Manifest:
    ...
    _raw_speaker_uri = ident_data.get('speakerUri', ident_data.get('serviceEndpoint', 'http://localhost:8080'))
    _raw_service_url = ident_data.get('serviceUrl', ident_data.get('serviceEndpoint', 'http://localhost:8080'))
    if _service_host:
